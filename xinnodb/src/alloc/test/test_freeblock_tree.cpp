...
struct MockBlock : public alloc_free_block_header {
    MockBlock(ib_u64 size) {
        this->this_desc.size = size;
        this->this_desc.state = (unsigned int)(ALLOC_BLOCK_STATE_FREE);
    }
};

MockBlock* create_mock_block(ib_u64 size) {
...
bool is_tree_node(const alloc_free_block_header* node) {
    return node->height >= 0;
}

int compute_height(const alloc_free_block_header* node) {
    if (!node) return 0;
    if (!is_tree_node(node)) return 0;
    return 1 + std::max(compute_height(node->left), compute_height(node->right));
}

int compute_balance(const alloc_free_block_header* node) {
    if (!node || !is_tree_node(node)) return 0;
    return compute_height(node->left) - compute_height(node->right);
}

void verify_tree(const alloc_free_block_header* node, ib_u64 min_key = 0, ib_u64 max_key = UINT64_MAX) {
    if (!node) return;
    ...
    if (!alloc_freeblock_is_detached(node)) {
        const alloc_free_block_header* current = node;
        int count = 0;
        do {
            ...
            ut_dlink* nl = current->multimap_link.next;
            current = (const alloc_free_block_header*)((const char*)nl - IB_OFFSET_OF(alloc_free_block_header, multimap_link));
            ++count;
...
TEST(AllocFreeBlockHeaderTest, Init) {
    alloc_free_block_header* root = reinterpret_cast<alloc_free_block_header*>(0xdeadbeef);
...
TEST(AllocFreeBlockHeaderTest, InsertSingle) {
    ...
    alloc_free_block_header* root = nullptr;

    MockBlock* b1 = create_mock_block(8192);
    blocks.push_back(b1);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b1));
    EXPECT_EQ(root, b1);
...
TEST(AllocFreeBlockHeaderTest, InsertDuplicate) {
    ...
    blocks.push_back(b2);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b2));
    EXPECT_EQ(root, b1);
...
TEST(AllocFreeBlockHeaderTest, InsertMultipleDuplicates) {
    ...
    blocks.push_back(b3);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b3));
...
TEST(AllocFreeBlockHeaderTest, InsertRightRotation) {
    ...
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b300));

    MockBlock* b200 = create_mock_block(16384);
    blocks.push_back(b200);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b200));

    MockBlock* b100 = create_mock_block(8192);
    blocks.push_back(b100);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b100));
...
TEST(AllocFreeBlockHeaderTest, FindGTE_Exact) {
    ...
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b64));

    MockBlock* b128 = create_mock_block(16384);
    blocks.push_back(b128);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b128));

    alloc_free_block_header* found = alloc_freeblock_find_gte(root, 16384);
...
TEST(AllocFreeBlockHeaderTest, FindGTE_Greater) {
    ...
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b256));
...
TEST(AllocFreeBlockHeaderTest, FindGTE_None) {
    alloc_free_block_header* root = nullptr;
    alloc_free_block_header* found = alloc_freeblock_find_gte(root, 8192);
    EXPECT_EQ(found, nullptr);

    MockBlock* b32 = create_mock_block(16384);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b32));
...
TEST(AllocFreeBlockHeaderTest, DetachTreeNodeOneChild) {
    ...
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b50));
...
TEST(AllocFreeBlockHeaderTest, DetachTreeNodeTwoChildren) {
    ...
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b150));

    MockBlock* b125 = create_mock_block(20480);
    blocks.push_back(b125);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b125));
...
TEST(AllocFreeBlockHeaderTest, FIFOOrder) {
    ...
    alloc_free_block_header* group = alloc_freeblock_find_gte(root, 8192);
    EXPECT_EQ(group, b1);
...
TEST(AllocFreeBlockHeaderTest, Clear) {
    ...
    b->height = 5;
    b->balance = 2;
    b->parent = reinterpret_cast<alloc_free_block_header*>(0x1);
    b->left = reinterpret_cast<alloc_free_block_header*>(0x2);
    b->right = reinterpret_cast<alloc_free_block_header*>(0x3);
    ...
    EXPECT_EQ((ib_u64)b->this_desc.size, (ib_u64)8192);
    EXPECT_EQ(b->this_desc.state, (unsigned int)(ALLOC_BLOCK_STATE_FREE));
...
TEST(AllocFreeBlockHeaderTest, IsDetached) {
    ...
    b->multimap_link.next = reinterpret_cast<ut_dlink*>(0x1);
    EXPECT_FALSE(alloc_freeblock_is_detached(b));

    b->multimap_link.prev = reinterpret_cast<ut_dlink*>(0x2);
    EXPECT_FALSE(alloc_freeblock_is_detached(b));
...
TEST(AllocFreeBlockHeaderTest, DetachTriggersRightRotation) {
    ...
    blocks.push_back(b4);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b4));
    ...
    blocks.push_back(b5);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b5));
...
TEST(AllocFreeBlockHeaderTest, DetachTriggersLeftRotation) {
    ...
    blocks.push_back(b0);
    alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b0));
...
TEST(AllocFreeBlockHeaderTest, LargeTreeMultipleOperations) {
    ...
    std::vector<ib_u64> sizes = {8192, 16384, 24576, 16384, 24576, 32768, 40960, 16384, 24576, 8192, 8192, 24576};

    for (ib_u64 s : sizes) {
        MockBlock* b = create_mock_block(s);
        blocks.push_back(b);
        alloc_freeblock_put(&root, reinterpret_cast<alloc_block_header*>(b));
        verify_tree(root);
    }

    for (size_t i = 0; i < blocks.size() / 2; ++i) {
        alloc_free_block_header* to_detach = alloc_freeblock_find_gte(root, 16384);
        if (to_detach) {
    ...
    while (root) {
        alloc_free_block_header* to_detach = root;
        alloc_freeblock_detach(&root, to_detach);
        verify_tree(root);
    }
