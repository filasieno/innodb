...
inline static int                     alloc_freeblock_height_of(const alloc_free_block_header* n) noexcept;
inline static void                    alloc_freeblock_update(alloc_free_block_header* n) noexcept;
inline static void                    alloc_freeblock_rotate_left(alloc_free_block_header** root, alloc_free_block_header* x) noexcept;
inline static void                    alloc_freeblock_rotate_right(alloc_free_block_header** root, alloc_free_block_header* y) noexcept;
inline static void                    alloc_freeblock_rebalance_upwards(alloc_free_block_header** root, alloc_free_block_header* n) noexcept;
inline static void                    alloc_freeblock_transplant(alloc_free_block_header** root, alloc_free_block_header* u, alloc_free_block_header* v) noexcept;
inline static alloc_free_block_header* alloc_freeblock_min_node(alloc_free_block_header* root) noexcept;

void alloc_freeblock_init_root(alloc_free_block_header** root) noexcept {
    IB_ASSERT(root != nullptr);
    *root = nullptr;
}
...
void alloc_freeblock_put(alloc_free_block_header** root, alloc_block_header* block) noexcept {
    ...
    IB_ASSERT(block->this_desc.size > 2048);

    auto key_of = [](const alloc_free_block_header* n) noexcept -> ib_u64 { return n->this_desc.size; };
    // (helpers moved to static inline utilities above)

    alloc_free_block_header* new_link = (alloc_free_block_header*)block;
    ...
    if (*root == nullptr) {
        ...
        ut_dlink_init(&new_link->multimap_link);
        *root = new_link;
        return;
    }

    // Traverse to find insertion point or existing key
    alloc_free_block_header* cur = *root;
    alloc_free_block_header* parent = nullptr;
    ib_u64 k = new_link->this_desc.size;
    while (cur) {
        parent = cur;
        ib_u64 ck = key_of(cur);
        if (k == ck) {
            ...
            ut_dlink_insert_prev(&cur->multimap_link, &new_link->multimap_link);
            return;
        } else if (k < ck) {
            cur = cur->left;
        } else {
...
alloc_free_block_header* alloc_freeblock_find_gte(alloc_free_block_header* root, ib_u64 block_size) noexcept {
    ...
    alloc_free_block_header* node = root;
    alloc_free_block_header* best = nullptr;
    while (node) {
        ib_u64 k = node->this_desc.size;
...
void alloc_freeblock_detach(alloc_free_block_header** root, alloc_free_block_header* node) noexcept {
    IB_ASSERT(root != nullptr);
    IB_ASSERT(*root != nullptr);
    IB_ASSERT(node != nullptr);
    IB_ASSERT(node->this_desc.state == (ib_u32)ALLOC_BLOCK_STATE_FREE);
    IB_ASSERT(node->this_desc.size > 2048);
    ...
    if (alloc_freeblock_is_detached(node)) {
        alloc_free_block_header* start_rebalance = node->parent;
        if (node->left == nullptr) {
            alloc_freeblock_transplant(root, node, node->right);
        } else if (node->right == nullptr) {
            alloc_freeblock_transplant(root, node, node->left);
        } else {
            alloc_free_block_header* s = alloc_freeblock_min_node(node->right);
            if (s->parent != node) {
                // Replace s with its right subtree
                alloc_free_block_header* sp = s->parent;
...
void alloc_freeblock_clear(alloc_free_block_header* link) noexcept {
    IB_ASSERT(link != nullptr);
    char* buff = ((char*)link) + sizeof(alloc_block_header);
    std::memset(buff, 0, sizeof(alloc_free_block_header) - sizeof(alloc_block_header));
}

inline static int alloc_freeblock_height_of(const alloc_free_block_header* n) noexcept { return n ? n->height : 0; }
...
inline static void alloc_freeblock_rotate_left(alloc_free_block_header** r, alloc_free_block_header* x) noexcept {
    alloc_free_block_header* y = x->right;
    IB_ASSERT(y != nullptr);
    x->right = y->left;
...
inline static void alloc_freeblock_rotate_right(alloc_free_block_header** r, alloc_free_block_header* y) noexcept {
    alloc_free_block_header* x = y->left;
    IB_ASSERT(x != nullptr);
    y->left = x->right;
...
inline static void alloc_freeblock_rebalance_upwards(alloc_free_block_header** r, alloc_free_block_header* n) noexcept {
    while (n) {
...
inline static void alloc_freeblock_transplant(alloc_free_block_header** r, alloc_free_block_header* u, alloc_free_block_header* v) noexcept {
    if (u->parent == nullptr) {
        *r = v;
...
inline static alloc_free_block_header* alloc_freeblock_min_node(alloc_free_block_header* n) noexcept {
    IB_ASSERT(n != nullptr);
