...
IB_INTERN void trx_sys_mark_upgraded_to_multiple_tablespaces(void)
{
	...
	// We upgraded to 4.1.x and reset the space id fields in the doublewrite
	// buffer. Let us mark to the trx_sys header that the upgrade has been done.
...
start_again:
	...
	buf_block_t *block =
		buf_page_get(TRX_SYS_SPACE, 0, TRX_SYS_PAGE_NO, RW_X_LATCH, &mtr);
	buf_block_dbg_add_level(block, SYNC_NO_ORDER_CHECK);

	byte *doublewrite = buf_block_get_frame(block) + TRX_SYS_DOUBLEWRITE;

	if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC) ==
		TRX_SYS_DOUBLEWRITE_MAGIC_N) {
		/* The doublewrite buffer has already been created: just read in some
     * numbers */
		trx_doublewrite_init(doublewrite);
		mtr_commit(&mtr);
		trx_doublewrite_buf_is_being_created = FALSE;
	} else {
		ib_log(state, "InnoDB: Doublewrite buffer not found: creating new\n");

		ulint required_size =
			(2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2 + 100) *
			IB_PAGE_SIZE;
		if (buf_pool_get_curr_size() < required_size) {
			ib_log(state, "InnoDB: Cannot create doublewrite buffer: you must\n");
			ib_log(state, "InnoDB: increase your buffer pool size.\n");
			ib_log(state, "InnoDB: Cannot continue operation.\n");
			return (DB_FATAL);
		}

		buf_block_t *block2 =
			fseg_create(TRX_SYS_SPACE, TRX_SYS_PAGE_NO, TRX_SYS_DOUBLEWRITE + TRX_SYS_DOUBLEWRITE_FSEG, &mtr);

		/* fseg_create acquires a second latch on the page, therefore we must
     * declare it: */
		buf_block_dbg_add_level(block2, SYNC_NO_ORDER_CHECK);

		if (block2 == NULL) {
			ib_log(state, "InnoDB: Cannot create doublewrite buffer: you must\n");
			ib_log(state, "InnoDB: increase your tablespace size.\n");
			ib_log(state, "InnoDB: Cannot continue operation.\n");
			/* We need to exit without committing the mtr to prevent its modifications
       * to the database getting to disk */
			return (DB_FATAL);
		}

		byte *fseg_header = buf_block_get_frame(block) + TRX_SYS_DOUBLEWRITE +
							TRX_SYS_DOUBLEWRITE_FSEG;
		ulint prev_page_no = 0;

		ulint num_pages = 2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2;
		for (ulint i = 0; i < num_pages; i++) {
			...
			/* We read the allocated pages to the buffer pool; when they are written
to disk in a flush, the space id and page number fields are also written to the
pages. When we at database startup read pages from the doublewrite buffer, we
know that if the space id and page number in them are the same as the page
position in the tablespace, then the page has not been written to in
doublewrite. */

			buf_block_t *new_block =
				buf_page_get(TRX_SYS_SPACE, 0, page_no, RW_X_LATCH, &mtr);
		...
		/* Flush the modified pages to disk and make a checkpoint */
		log_make_checkpoint_at(IB_UINT64_T_MAX, TRUE);
		ib_log(state, "InnoDB: Doublewrite buffer created\n");
		trx_sys_multiple_tablespace_format = TRUE;
		goto start_again;
	}
...
) /*!< in: recovery flag */
{
  ...
  the database is repeatedly started! */
	...
	if (UT_LIST_GET_LEN(trx_sys->trx_list) > 0) {
		...
		ib_log(state, "InnoDB: Trx id counter is " TRX_ID_FMT "\n", TRX_ID_PREP_PRINTF(trx_sys->max_trx_id));
	}
...
@return	always TRUE */
static ibool trx_sys_file_format_max_write(
	/*==========================*/
	ulint format_id, /*!< in: file format id */
	const char **name
) /*!< out: max file format name, can
                                                             be NULL */
{
	mtr_t mtr;
	byte *ptr;
...
) /*!< in: pointer to file format name */
{
	char *endp;
	ulint format_id;
...
) /*!< in: max format id to check */
{
  ...
  unless forced by the user. */
	...
	ib_log(state, "  InnoDB: highest supported file format is %s.\n", trx_sys_file_format_id_to_name(DICT_TF_FORMAT_MAX));
