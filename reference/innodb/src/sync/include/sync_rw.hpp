...
/// \file include/sync_rw.h
/// The read-write lock (for threads, not for database transactions)
/// \details Originally created by 9/11/1995 Heikki Tuuri
...
constinit ulint RW_S_LATCH  = 1;
constinit ulint RW_X_LATCH  = 2;
constinit ulint RW_NO_LATCH	= 3;
...
/** number of unlocks (that unlock shared locks), set only when IB_SYNC_PERF_STAT is defined */
...
/** number of unlocks (that unlock exclusive locks), set only when IB_SYNC_PERF_STAT is defined */
...
to the non-locked state. Explicit freeing of the rw-lock with rw_lock_free
is necessary only if the memory block containing it is freed. */
IB_INTERN void rw_lock_create_func(
...
/// Removes an rw-lock object from the global list.
...
IB_INTERN void rw_lock_free(rw_lock_t* lock);
	...
	/// \return	TRUE 
	IB_INTERN ibool rw_lock_validate(rw_lock_t*	lock);	/*!< in: rw-lock */
#endif // IB_DEBUG

/** NOTE! The following macros should be used in rw s-locking, not the corresponding function. */
...
/// \return TRUE
IB_INLINE ibool rw_lock_s_lock_low(
	rw_lock_t*	lock,	
...
time (controlled by state->srv.n_spin_wait_rounds), waiting for the lock, before
suspending the thread. */
IB_INLINE void rw_lock_s_lock_func(
...
IB_INLINE ibool rw_lock_x_lock_func_nowait(
	rw_lock_t*	lock,	   /*!< in: pointer to rw-lock */
	const char*	file_name, /*!< in: file name where lock requested */
	ulint		line);	   /*!< in: line where requested */
...
/** NOTE! The following macro should be used in rw x-locking, not the corresponding function. */
...
/** NOTE! The following macros should be used in rw x-locking, not the corresponding function. */
...
IB_INTERN void rw_lock_x_lock_func(
	rw_lock_t*	lock,	/*!< in: pointer to rw-lock */
	ulint		pass,	/*!< in: pass value; != 0, if the lock will be passed to another thread to unlock */
	const char*	file_name,/*!< in: file name where lock requested */
	ulint		line);	/*!< in: line where requested */
...
IB_INLINE void rw_lock_s_lock_direct(
	rw_lock_t*	lock,		/*!< in/out: rw-lock */
...
Then we can do the locking without reserving the mutex. */
IB_INLINE void rw_lock_x_lock_direct(
	rw_lock_t*	lock,		/*!< in/out: rw-lock */
	const char*	file_name,	/*!< in: file name where requested */
	ulint		line);		/*!< in: line where lock requested */
...
operations. */
IB_INTERN void rw_lock_x_lock_move_ownership(rw_lock_t*	lock);	/*!< in: lock which was x-locked in the buffer read */
...
IB_INLINE void rw_lock_s_unlock_direct(rw_lock_t* lock);	/*!< in/out: rw-lock */

/** Releases an exclusive mode lock when we know there are no waiters, and none else will access the lock durint the time this function is executed. */
IB_INLINE void rw_lock_x_unlock_direct(rw_lock_t* lock);	/*!< in/out: rw-lock */

/** Returns the value of writer_count for the lock. Does not reserve the lock mutex, so the caller must be sure it is not changed during the call. @return	value of writer_count */
IB_INLINE ulint rw_lock_get_x_lock_count(const rw_lock_t* lock);	/*!< in: rw-lock */

/**
Check if there are threads waiting for the rw-lock.
@return	1 if waiters, 0 otherwise */
IB_INLINE ulint rw_lock_get_waiters(const rw_lock_t* lock);	/*!< in: rw-lock */

/**
Returns the write-status of the lock - this function made more sense
with the old rw_lock implementation.
@return	RW_LOCK_NOT_LOCKED, RW_LOCK_EX, RW_LOCK_WAIT_EX */
IB_INLINE ulint rw_lock_get_writer(const rw_lock_t*	lock);	/*!< in: rw-lock */

/** Returns the number of readers.
@return	number of readers */
IB_INLINE ulint rw_lock_get_reader_count(const rw_lock_t* lock); /*!< in: rw-lock */

/**
Decrements lock_word the specified amount if it is greater than 0.
This is used by both s_lock and x_lock operations.
@return	TRUE if decr occurs */
IB_INLINE ibool rw_lock_lock_word_decr(
	rw_lock_t*	lock,		/*!< in/out: rw-lock */
...
IB_INTERN ibool rw_lock_own(
	rw_lock_t*	lock,		/*!< in: rw-lock */
	ulint		lock_type)	/*!< in: lock type: RW_LOCK_SHARED, RW_LOCK_EX */
	__attribute__((warn_unused_result));
#endif // IB_SYNC_DEBUG 

/** Checks if somebody has locked the rw-lock in the specified mode. */
IB_INTERN ibool rw_lock_is_locked(
	rw_lock_t*	lock,		/*!< in: rw-lock */
...
IB_INTERN void rw_lock_debug_print(rw_lock_debug_t*	info);	/*!< in: debug struct */
