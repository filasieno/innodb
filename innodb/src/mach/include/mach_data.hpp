...
UNIV_INLINE ulint mach_read_from_2(const byte* b) __attribute__((nonnull, pure));
...
UNIV_INLINE ib_uint16_t mach_encode_2(ulint n) __attribute__((const));
...
UNIV_INLINE ulint mach_decode_2(ib_uint16_t n) __attribute__((const));
...
/// \return void
UNIV_INLINE void mach_write_to_3(byte* b, ulint n) __attribute__((nonnull));
...
UNIV_INLINE ulint mach_read_from_3(const byte* b) __attribute__((nonnull, pure));
...
/// \param [in] n ulint integer to be stored
/// \return void
UNIV_INLINE void mach_write_to_4(byte* b, ulint n) __attribute__((nonnull));
...
/// \return ulint integer
UNIV_INLINE ulint mach_read_from_4(const byte* b) __attribute__((nonnull, pure));
...
UNIV_INLINE ulint mach_write_compressed(byte* b, ulint n) __attribute__((nonnull));
...
/// \return read integer
UNIV_INLINE ulint mach_read_compressed(const byte* b) __attribute__((nonnull, pure));
...
UNIV_INLINE dulint mach_read_from_6(const byte* b) __attribute__((nonnull, pure));
...
/// \return dulint integer
UNIV_INLINE dulint mach_read_from_7(const byte* b) __attribute__((nonnull, pure));
...
/// \return dulint integer
UNIV_INLINE dulint mach_read_from_8(const byte* b) __attribute__((nonnull, pure));

/// \brief The following function is used to fetch data from 8 consecutive bytes.
/// The most significant byte is at the lowest address.
/// \param [in] pointer to 8 bytes
/// \return 64-bit integer 
UNIV_INLINE ib_uint64_t mach_read_ull(const byte* b) __attribute__((nonnull, pure));
...
/// \param [in] b pointer to memory from where to read
/// \return read dulint 
UNIV_INLINE dulint mach_dulint_read_compressed(const byte* b) __attribute__((nonnull, pure));

/// \brief Writes a dulint in a compressed form (1..11 bytes).
/// \param [in] b pointer to memory where to store
/// \param [in] n integer to be stored 
/// \return size in bytes 
UNIV_INLINE ulint mach_dulint_write_much_compressed(byte* b, dulint n); /*!< in: */

/// \brief Returns the size of a dulint when written in the compressed form.
/// \return compressed size in bytes 
/// \param [in] n dulint integer to be stored
UNIV_INLINE ulint mach_dulint_get_much_compressed_size(dulint n) __attribute__((const));

/// \brief Reads a dulint in a compressed form.
/// \param [in] b pointer to memory from where to read
/// \return read dulint
UNIV_INLINE dulint mach_dulint_read_much_compressed(const byte* b) __attribute__((nonnull, pure));
...
UNIV_INTERN byte* mach_dulint_parse_compressed(byte* ptr, byte* end_ptr, dulint* val);
	...
	UNIV_INLINE double mach_double_read(const byte* b) __attribute__((nonnull, pure));
	...
	/// \param [in] b pointer to memory from where to read
	/// \return float read
	UNIV_INLINE float mach_float_read(const byte* b) __attribute__((nonnull, pure));

	/// \brief Writes a pointer to float. It is stored in a little-endian format.
	/// \param [in] b pointer to memory where to write
	/// \param [in] p pointer to float
	UNIV_INLINE void mach_float_ptr_write(byte* b, const byte* p);
	...
	UNIV_INLINE ulint mach_read_from_n_little_endian(const byte* buf, ulint buf_size) __attribute__((nonnull, pure));
	...
	/// \param [in] buf from where to read
	/// \return unsigned long int
	UNIV_INLINE ulint mach_read_from_2_little_endian(const byte* buf) __attribute__((nonnull, pure));
	...
	UNIV_INLINE void mach_read_int_type(void* dst, const byte* src, ulint len, ibool unsigned_type);
	...
	/// \param [in] len length of src
	/// \param [in] unsigned_type signed or unsigned flag
	UNIV_INLINE void mach_write_int_type(byte* dest, const byte* src, ulint len, ibool unsigned_type);
	...
	UNIV_INLINE void mach_write_uint64(byte* dest, ib_uint64_t n);
