...
constinit ulint BTR_PAGE_MAX_REC_SIZE = (IB_PAGE_SIZE / 2 - 200);

/// \brief Maximum depth of a B-tree in InnoDB.
/// \details Note that this isn't a maximum as such; none of the tree operations
...
/// \brief If this is ORed to btr_latch_mode, it means that the search tuple will be inserted to the index, at the searched position
constinit ulint BTR_INSERT = 512;

/// \brief This flag ORed to btr_latch_mode says that we do the search in query optimization
constinit ulint BTR_ESTIMATE = 1024;

/// \brief This flag ORed to btr_latch_mode says that we can ignore possible UNIQUE definition on secondary indexes when we decide if we can use the insert buffer to speed up inserts
constinit ulint BTR_IGNORE_SEC_UNIQUE = 2048;
...
/// \param [in] page page to be created
...
/// \param [in] page index page
/// \param [in] page_zip compressed page whose uncompressed part will be updated, or NULL
/// \param [in] level level, leaf level == 0
/// \param [in] mtr mini-transaction handle
...
/// \param [in] page index page
/// \param [in] page_zip compressed page whose uncompressed part will be updated, or NULL
/// \param [in] prev previous page number
/// \param [in] mtr mini-transaction handle
