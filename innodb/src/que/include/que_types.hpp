...
/// \brief Pseudotype for all graph nodes
typedef void que_node_t;

typedef struct que_fork_struct que_fork_t;

/// \brief Query graph root is a fork node
...
/// \brief Common struct at the beginning of each query graph node; the name of this substruct must be 'common'.
...
struct que_common_struct {
	...
	ulint val_buf_size;  /*!< buffer size for the evaluated value data, if the buffer has been allocated dynamically: if this field is != 0, and the node is a symbol node or a function node, then we have to free the data field in val explicitly */
};
