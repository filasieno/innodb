...
/// \param [in] query_fork query fork
IB_INLINE que_thr_t* que_fork_get_first_thr(que_fork_t* query_fork)
{
...
/// \param [in] query_fork query fork
IB_INLINE que_node_t* que_fork_get_child(que_fork_t* query_fork)
{
    que_thr_t* thr = UT_LIST_GET_FIRST(query_fork->thrs);
    return thr->child;
}
...
{
    que_common_t* cnode = (que_common_t*) node;
    cnode->brother = NULL;
    if (node_list == NULL) {
        return node;
    }
    que_common_t* cnode2 = (que_common_t*) node_list;
...
{
    const que_common_t* cnode = (const que_common_t*) node_list;
    ulint len = 0;
    while (cnode != NULL) {
        len++;
...
/// \details NOTE that if the peek is made without reserving the kernel mutex, then another peek with the mutex reserved is necessary before deciding the actual stopping
