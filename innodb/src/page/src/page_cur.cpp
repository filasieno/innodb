...
/// \brief This is a linear congruential generator PRNG.
/// \details Returns a pseudo random number between 0 and 2^64-1 inclusive. The formula and the constants being used are: X[n+1] = (a * X[n] + c) mod m where: X[0] = ut_time_us(NULL) a = 1103515245 (3^5 * 5 * 7 * 129749) c = 12345 (3 * 5 * 823) m = 18446744073709551616 (2^64)
...
/// \brief Checks if the nth field in a record is a character type field which extends the nth field in tuple, i.e., the field is longer or equal in length and has common first characters.
...
/// \brief Positions a page cursor on a randomly chosen user record on a page. If there are no user records, sets the cursor on the infimum record.
...
IB_INTERN byte* page_cur_parse_insert_rec(ibool is_short, byte* ptr, byte* end_ptr, buf_block_t* block, dict_index_t* index, mtr_t* mtr)
{
    ...
    offsets = rec_get_offsets(buf + origin_offset, index, offsets,
                  ULINT_UNDEFINED, &heap);
    if (IB_UNLIKELY(!page_cur_rec_insert(&cursor,
                           buf + origin_offset,
                           index, offsets, mtr))) {
...
use_heap:
    ...
    IB_MEM_ASSERT_RW(rec_get_start(insert_rec, offsets),
