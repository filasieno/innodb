...
page_cur_set_before_first(
	const buf_block_t* block, page_cur_t* cur)
{
...
void
page_cur_position(
	const rec_t* rec, const buf_block_t* block, page_cur_t* cur)
{
    ut_ad(rec && block && cur);
	...
	const buf_block_t* block, const dict_index_t* dict_index, const dtuple_t* tuple, ulint mode, page_cur_t* cursor)
{
    ...
    page_cur_search_with_match(block, dict_index, tuple, mode,
                               &up_matched_fields,
                               &up_matched_bytes,
                               &low_matched_fields,
                               &low_matched_bytes,
...
page_cur_tuple_insert(
	page_cur_t* cursor, const dtuple_t* tuple, dict_index_t* dict_index, ulint n_ext, mtr_t* mtr)
{
    ...
    mem_heap_t* heap = mem_heap_create(size
                                       ...
                                       * sizeof *offsets);
    rec_t* rec = rec_convert_dtuple_to_rec((byte*) mem_heap_alloc(heap, size),
                                           dict_index, tuple, n_ext);
