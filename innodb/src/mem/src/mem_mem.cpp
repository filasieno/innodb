...
/// \file mem/src/mem_mem.cpp
/// \brief The memory management
/// \details Originally created by Heikki Tuuri on 6/9/1994
...
/// The heap is physically arranged so that if the current block becomes full, a new block is allocated and always inserted in the chain of blocks as the last block.
...
/// The start field has the following content: A. sizeof(ulint) bytes of field length (in the standard byte order) B. sizeof(ulint) bytes of check field (a random number)
/// The trailer field contains: A. sizeof(ulint) bytes of check field (the same random number as at the start)
/// Thus we can notice if something has been copied over the borders of the buffer, which is illegal. The memory in the buffers is initialized to a random byte sequence. After freeing, all the blocks in the heap are set to random bytes to help us discover errors which result from the use of buffers in an already freed heap.
...
/// \param buf in/out: buffer to store formatted string in, or NULL to just calculate length
/// \param format in: format string
/// \param ap in: arguments
...
/// \brief Prints the contents of a memory heap.
/// \param heap in: memory heap
static void mem_heap_print(mem_heap_t *heap);
...
/// \param heap in: memory heap where copy is allocated
...
IB_INTERN char *mem_heap_strcat(mem_heap_t *heap, const char *s1, const char *s2)
{
	...
	char *s = mem_heap_alloc(heap, s1_len + s2_len + 1);
...
IB_INTERN char *mem_heap_printf(mem_heap_t *heap, const char *format, ...)
{
	va_list ap;
	ulint len;
	...
	char *str = mem_heap_alloc(heap, len);
	va_start(ap, format);
...
/// \param heap in: memory heap or NULL if first block should be created
/// \param n in: number of bytes needed for user data
/// \param type in: type of heap: MEM_HEAP_DYNAMIC or MEM_HEAP_BUFFER
/// \param file_name in: file name where created
...
IB_INTERN mem_block_t *mem_heap_create_block(mem_heap_t *heap, ulint n, ulint type, const char *file_name, ulint line)
{
	...
	ulint len = MEM_BLOCK_HEADER_SIZE + MEM_SPACE_NEEDED(n);
...
IB_INTERN mem_block_t *mem_heap_add_block(mem_heap_t *heap, ulint n)
{
	...
	// We have to allocate a new block. The size is always at least doubled until the standard size is reached. After that the size stays the same, except in cases where the caller needs more space.
	ulint new_size = 2 * mem_block_get_len(block);
...
/// of blocks.
/// \param heap in: memory heap
/// \param top in: calculate and validate only until this top pointer in the heap is reached, if this pointer is NULL, ignored
/// \param print in: if TRUE, prints the contents of the heap; works only in the debug version
/// \param error out: TRUE if error
/// \param us_size out: allocated memory (for the user) in the heap, if a NULL pointer is passed as this argument, it is ignored; in the non-debug version this is always -1
/// \param ph_size out: physical size of the heap, if a NULL pointer is passed as this argument, it is ignored
/// \param n_blocks out: number of blocks in the heap, if a NULL pointer is passed as this argument, it is ignored
	...
	*error = TRUE;
	...
	while (block != NULL) {
		...
		if ((block->type == MEM_HEAP_BUFFER) && (mem_block_get_len(block) > IB_PAGE_SIZE)) {
			state->log(state->stream, "InnoDB: Error: mem block %p" " length %lu > IB_PAGE_SIZE\n", (void *)block, (ulong)mem_block_get_len(block));
			// error
			return;
		}

#ifdef IB_MEM_DEBUG
		// We can trace the fields of the block only in the debug version
		if (print) {
		...
		while (field < (byte *)block + mem_block_get_free(block)) {
			// Calculate the pointer to the storage which was given to the user
		...
		// At the end check that we have arrived to the first free position

		if (field != (byte *)block + mem_block_get_free(block)) {
			// error
			state->log(state->stream, "InnoDB: Error: block %lx end of" " mem fields %lx\n" "InnoDB: but block free at %lx\n", (ulint)block, (ulint)field, (ulint)((byte *)block + mem_block_get_free(block)));
			return;
		}
