...
/// Originally created 9/5/1995 Heikki Tuuri
...
/// \details Reserves a mutex for the current thread. 
/// If the mutex is reserved, the function spins a preset time (controlled by state->srv.n_spin_wait_rounds) waiting
...
IB_INTERN void mutex_set_debug_info(ib_mutex_t* mutex, const char* file_name, ulint line);

/// \brief Releases the threads waiting in the primary wait array for this mutex.
IB_INTERN void mutex_signal_object(ib_mutex_t* mutex);

/// \brief Performs an atomic test-and-set instruction to the lock_word field of a mutex.
/// \param [in, out] mutex Pointer to mutex
/// \return the previous value of lock_word: 0 or 1
IB_INLINE ibool mutex_test_and_set( ib_mutex_t* mutex)
{
...
/// \brief Performs a reset instruction to the lock_word field of a mutex. 
/// This instruction also serializes memory operations to the program order.
/// \param [in, out] mutex Pointer to mutex
IB_INLINE void mutex_reset_lock_word(ib_mutex_t* mutex)
{
	if constexpr (IB_HAVE_ATOMIC_BUILTINS) {
...
/// \param [in] mutex Pointer to mutex
/// \return	value to set
IB_INLINE ulint mutex_get_waiters(const ib_mutex_t* mutex)
{
	ut_ad(mutex);
...
IB_INLINE void mutex_enter_func(ib_mutex_t* mutex, const char* file_name, ulint line)
{
