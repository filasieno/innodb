...
/// \brief This value is used by default by ha_storage_create(). More memory is allocated later when/if it is needed
constinit ulint HA_STORAGE_DEFAULT_HEAP_BYTES = 1024;

/// \brief This value is used by default by ha_storage_create(). It is a constant per ha_storage's lifetime.
