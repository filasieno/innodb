...
/// \file log_log.cpp
/// \brief Database log implementation
/// \details Originally created by Heikki Tuuri in 12/9/1995
...
// Current free limit of space 0; protected by the log sys mutex; 0 means uninitialized
IB_INTERN ulint log_fsp_current_free_limit = 0;
...
constinit ulint LOG_BUF_WRITE_MARGIN = (4 * OS_FILE_LOG_BLOCK_SIZE);

// Margins for free space in the log buffer after a log entry is catenated
constinit ulint LOG_BUF_FLUSH_RATIO = 2;
constinit ulint LOG_BUF_FLUSH_MARGIN = (LOG_BUF_WRITE_MARGIN + 4 * IB_PAGE_SIZE);

// Margin for the free space in the smallest log group, before a new query
// step which modifies the database, is started
constinit ulint LOG_CHECKPOINT_FREE_PER_THREAD = (4 * IB_PAGE_SIZE);
constinit ulint LOG_CHECKPOINT_EXTRA_FREE = (8 * IB_PAGE_SIZE);

// This parameter controls asynchronous making of a new checkpoint; the value
// should be bigger than LOG_POOL_PREFLUSH_RATIO_SYNC
constinit ulint LOG_POOL_CHECKPOINT_RATIO_ASYNC = 32;

// This parameter controls synchronous preflushing of modified buffer pages
constinit ulint LOG_POOL_PREFLUSH_RATIO_SYNC = 16;

// The same ratio for asynchronous preflushing; this value should be less than the previous
constinit ulint LOG_POOL_PREFLUSH_RATIO_ASYNC = 8;

// Extra margin, in addition to one log file, used in archiving
constinit ulint LOG_ARCHIVE_EXTRA_MARGIN = (4 * IB_PAGE_SIZE);

// This parameter controls asynchronous writing to the archive
constinit ulint LOG_ARCHIVE_RATIO_ASYNC = 16;

// Codes used in unlocking flush latches
constinit ulint LOG_UNLOCK_NONE_FLUSHED_LOCK = 1;
constinit ulint LOG_UNLOCK_FLUSH_LOCK = 2;

// States of an archiving operation
constinit ulint LOG_ARCHIVE_READ = 1;
constinit ulint LOG_ARCHIVE_WRITE = 2;
...
/// \brief Sets the global variable log_fsp_current_free_limit. Also makes a checkpoint, so that we know that the limit has been written to a log checkpoint field on disk.
...
/// \brief Returns the oldest modified block lsn in the pool, or log_sys->lsn if none exists.
/// \return LSN of oldest modification
static ib_uint64_t log_buf_pool_get_oldest_modification(void)
{
    ut_ad(mutex_own(&(log_sys->mutex)));
    ib_uint64_t lsn = buf_pool_get_oldest_modification();
    if (!lsn) {
        lsn = log_sys->lsn;
    }
    return lsn;
...
/// \brief Opens the log for log_write_low. The log must be closed with log_close and released with log_release.
...
loop:
    ...
    ulint len_upper_limit = LOG_BUF_WRITE_MARGIN + (5 * len) / 4;
    ...
    if (log->archiving_state != LOG_ARCH_OFF) {
        ulint archived_lsn_age = log->lsn - log->archived_lsn;
        if (archived_lsn_age + len_upper_limit > log->max_archived_lsn_age) {
            // Not enough free archived space in log groups: do a synchronous archive write batch:
            log_release();
...
/// \brief Writes to the log the string given. It is assumed that the caller holds the log mutex.
/// \param [in] str string
...
part_loop:
    ...
    byte *log_block = ut_align_down(log->buf + log->buf_free, OS_FILE_LOG_BLOCK_SIZE);
...
IB_INTERN ib_uint64_t log_close(ib_recovery_t recovery)
{
    ...
    ib_uint64_t lsn = log->lsn;
    byte *log_block = ut_align_down(log->buf + log->buf_free, OS_FILE_LOG_BLOCK_SIZE);
    ulint first_rec_group = log_block_get_first_rec_group(log_block);

    if (first_rec_group == 0) {
        // We initialized a new log block which was not written full by the current mtr: the next mtr log record group will start within this block at the offset data_len
    ...
    ib_uint64_t checkpoint_age = lsn - log->last_checkpoint_lsn;

    if (checkpoint_age >= log->log_group_capacity) {
        // TODO: split btr_store_big_rec_extern_fields() into small steps so that we can release all latches in the middle, and call log_free_check() to ensure we never write over log written after the latest checkpoint. In principle, we should split all big_rec operations, but other operations are smaller.
        if (!log_has_printed_chkp_warning || difftime(time(NULL), log_last_warning_time) > 15) {
            log_has_printed_chkp_warning = TRUE;
            log_last_warning_time = time(NULL);
            ut_print_timestamp(state->stream);
            state->log(state->stream, "  InnoDB: ERROR: the age of the last checkpoint is %lu,\nInnoDB: which exceeds the log group capacity %lu.\nInnoDB: If you are using big BLOB or TEXT rows, you must set the\nInnoDB: combined size of log files at least 10 times bigger than the\nInnoDB: largest such row.\n", (ulong)checkpoint_age, (ulong)log->log_group_capacity);
        }
    }
    ...
    ib_uint64_t oldest_lsn = buf_pool_get_oldest_modification();
    if (!oldest_lsn || lsn - oldest_lsn > log->max_modified_age_async || checkpoint_age > log->max_checkpoint_age_async) {
        log->check_flush_or_checkpoint = TRUE;
    }
function_exit:
...
#ifdef IB_LOG_ARCHIVE
/// \brief Pads the current log block full with dummy log records. Used in producing consistent archived log files.
/// \param [in] recovery recovery flag
static void log_pad_current_log_block(ib_recovery_t recovery)
{
    byte b = MLOG_DUMMY_RECORD;
    // We retrieve lsn only because otherwise gcc crashed on HP-UX
    ib_uint64_t lsn = log_reserve_and_open(OS_FILE_LOG_BLOCK_SIZE);
    ulint pad_length = OS_FILE_LOG_BLOCK_SIZE - (log_sys->buf_free % OS_FILE_LOG_BLOCK_SIZE) - LOG_BLOCK_TRL_SIZE;
...
/// \brief Calculates the offset within a log group, when the log file headers are not included.
/// \param [in] offset real offset within the log group
/// \param [in] group log group
/// \return size offset (<= offset)
IB_INLINE ulint log_group_calc_size_offset(ulint offset, const log_group_t *group)
{
	ut_ad(mutex_own(&(log_sys->mutex)));
...
/// \brief Calculates the offset within a log group, when the log file headers are included.
/// \param [in] offset size offset within the log group
/// \param [in] group log group
/// \return real offset (>= offset)
IB_INLINE ulint log_group_calc_real_offset(ulint offset, const log_group_t *group)
{
	ut_ad(mutex_own(&(log_sys->mutex)));
...
/// \brief Calculates the offset of an lsn within a log group.
/// \param [in] lsn lsn, must be within 4 GB of group->lsn
/// \param [in] group log group
/// \return offset within the log group
static ulint log_group_calc_lsn_offset(ib_uint64_t lsn, const log_group_t *group)
{
	ib_uint64_t gr_lsn;
	...
	gr_lsn_size_offset = (ib_int64_t)log_group_calc_size_offset(group->lsn_offset, group);
...
IB_INTERN ulint log_calc_where_lsn_is(ib_int64_t *log_file_offset, ib_uint64_t first_header_lsn, ib_uint64_t lsn, ulint n_log_files, ib_int64_t log_file_size)
{
	...
	if (lsn < first_header_lsn) {
		add_this_many = 1 + (first_header_lsn - lsn) / (capacity * (ib_int64_t)n_log_files);
...
#ifndef IB_HOTBACKUP
/// \brief Sets the field values in group to correspond to a given lsn. For this function to work, the values must already be correctly initialized to correspond to some lsn, for instance, a checkpoint lsn.
...
/// \brief Calculates the recommended highest values for lsn - last_checkpoint_lsn, lsn - buf_get_oldest_modification(), and lsn - max_archive_lsn_age.
...
static ibool log_calc_max_ages(void)
{
	...
	// For each OS thread we must reserve so much free space in the smallest log group that it can accommodate the log entries produced by single query steps: running out of free log space is a serious system error which requires rebooting the database.
...
IB_INTERN void innobase_log_init(void)
{
	...
	// Start the lsn from one log block from zero: this way every log record has a start lsn != zero, a fact which we will use
...
IB_INTERN void log_group_init(ulint id, ulint n_files, ulint file_size, ulint space_id, ulint archive_space_id __attribute__((unused)))
{
	ulint i;
...
/// \brief Checks if a flush is completed for a log group and does the completion routine if yes.
/// \param [in] group log group
/// \return LOG_UNLOCK_NONE_FLUSHED_LOCK or 0
IB_INLINE ulint log_group_check_flush_completion(log_group_t *group)
{
	ut_ad(mutex_own(&(log_sys->mutex)));
...
/// \brief Checks if a flush is completed and does the completion routine if yes.
...
/// \param [in] group log group or a dummy pointer
...
/// \brief Writes a log file header to a log file space.
/// \param [in] group log group
/// \param [in] nth_file header to the nth file in the log file space
/// \param [in] start_lsn log file data starts at this lsn
static void log_group_file_header_flush(log_group_t *group, ulint nth_file, ib_uint64_t start_lsn)
{
	byte *buf;
	ulint dest_offset;
...
/// \brief Stores a 4-byte checksum to the trailer checksum field of a log block before writing it to a log file. This checksum is used in recovery to check the consistency of a log block.
/// \param [in,out] block pointer to a log block
...
#endif	  // IB_DEBUG
	// Calculate the checksums for each log block and write them to the trailer fields of the log blocks
...
/// \brief This function is called, e.g., when a transaction wants to commit. It checks that the log has been written to the log file up to the last log entry written by the transaction. If there is a flush running, it waits and checks if the flush flushed enough. If not, starts a new flush.
...
IB_INTERN void log_write_up_to(ib_uint64_t lsn, ulint wait, ibool flush_to_disk)
{
	...
	if (recv_no_ibuf_operations) {
		// Recovery is running and no operations on the log files are allowed yet (the variable name .._no_ibuf_.. is misleading)
...
loop:
	...
	while (group) {
		log_group_write_buf(group, log_sys->buf + area_start, area_end - area_start, ut_uint64_align_down(log_sys->written_to_all_lsn, OS_FILE_LOG_BLOCK_SIZE), start_offset - area_start);
...
/// \brief This functions writes the log buffer to the log file and if 'flush' is set it forces a flush of the log file as well. This is meant to be called from background master thread only as it does not wait for the write (+ possible flush) to finish.
...
/// \brief Tries to establish a big enough margin of free space in the log buffer, such that a new log entry can be catenated without an immediate need for a flush.
...
void log_group_read_checkpoint_info(
	...
	ulint field
)	 // !< in: LOG_CHECKPOINT_1 or LOG_CHECKPOINT_2
{
	ut_ad(mutex_own(&(log_sys->mutex)));
...
) /*!< in: TRUE if we want to increment
					the file count */
{
	log_group_t *group;
	ulint trunc_len;
...
// ========================
{
	...
	if (log_sys->archiving_state == LOG_ARCH_OFF) {
		...
		log_sys->archived_lsn = ut_uint64_align_down(log_sys->lsn, OS_FILE_LOG_BLOCK_SIZE);
		log_release();
...
loop:
	...
	if (trx_n_transactions > 0 || (trx_sys != NULL && UT_LIST_GET_LEN(trx_sys->trx_list) > 0)) {
