...
/**
 * @brief Sets the global variable log_fsp_current_free_limit. Also makes a checkpoint,
 * so that we know that the limit has been written to a log checkpoint field on disk.
 */
IB_INTERN void log_fsp_current_free_limit_set_and_checkpoint(ulint limit) /**< in: limit to set */
{
	ibool success;
 ...
 */
IB_INTERN
ib_uint64_t log_reserve_and_open(ulint len) /**< in: length of data to be catenated */
{
	log_t *log = log_sys;
...
void log_flush_do_unlocks(
	// =================
	ulint code
) /*!< in: any ORed combination of LOG_UNLOCK_FLUSH_LOCK
...
void log_io_complete(
	// ============
	log_group_t *group
)	 // !< in: log group or a dummy pointer
{
	ulint unlock;
