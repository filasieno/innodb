...
/** The recovery system */
IB_INTERN recv_sys_t *recv_sys = NULL;
...
IB_INTERN ulint recv_max_parsed_page_no;
...
IB_INTERN ulint recv_n_pool_free_frames;

/** The maximum lsn we see for a page during the recovery process. If this
is bigger than the lsn we are able to scan up to, that is an indication that
the recovery failed and the database may be corrupt. */
IB_INTERN ib_uint64_t recv_max_page_lsn;
...
// =================
{
	...
	recv_sys = IB_MEM_ALLOC(sizeof(*recv_sys));
...
// ===================
{
	if (recv_sys != NULL) {
		...
		if (recv_sys->heap != NULL) {
			IB_MEM_HEAP_FREE(recv_sys->heap);
		}
		...
		if (recv_sys->last_block_buf_start != NULL) {
			IB_MEM_FREE(recv_sys->last_block_buf_start);
		}

		IB_MEM_FREE(recv_sys);
		recv_sys = NULL;
	}
}
...
)	 // !< in: available memory in bytes
{
	...
	recv_sys->heap = IB_MEM_HEAP_CREATE_IN_BUFFER(256);
#else	  // !IB_HOTBACKUP
	recv_sys->heap = IB_MEM_HEAP_CREATE(256);
	...
	recv_sys->last_block_buf_start = IB_MEM_ALLOC(2 * OS_FILE_LOG_BLOCK_SIZE);
					...
					
{
	...
	finish_lsn1 = ut_uint64_align_down(archived_lsn, OS_FILE_LOG_BLOCK_SIZE) + log_group_get_capacity(group);

	finish_lsn2 = ut_uint64_align_up(recovered_lsn, OS_FILE_LOG_BLOCK_SIZE) + recv_sys->last_log_buf_size;
...
static ibool recv_check_cp_is_consistent(
	// ========================
	const byte *buf
)	 // !< in: buffer containing checkpoint info
{
	ulint fold;
					...
					data ended here */
{
	...
	*n_bytes_scanned = 0;
	...
	for (log_block = buf; log_block < buf + buf_len; log_block += OS_FILE_LOG_BLOCK_SIZE) {
		...
		if (*scanned_checkpoint_no > 0 && log_block_get_checkpoint_no(log_block) < *scanned_checkpoint_no && *scanned_checkpoint_no - log_block_get_checkpoint_no(log_block) > 0x80000000UL) {
	...
	switch (type) {
		...
		case MLOG_COMP_REC_INSERT:
			...
			if (NULL != (ptr = mlog_parse_index(ptr, end_ptr, type == MLOG_COMP_REC_INSERT, &index))) {
				ut_a(!page || (ibool) !!page_is_comp(page) == dict_table_is_comp(index->table));
		...
		case MLOG_COMP_REC_CLUST_DELETE_MARK:
			...
			if (NULL != (ptr = mlog_parse_index(ptr, end_ptr, type == MLOG_COMP_REC_CLUST_DELETE_MARK, &index))) {
				ut_a(!page || (ibool) !!page_is_comp(page) == dict_table_is_comp(index->table));
				ptr = btr_cur_parse_del_mark_set_clust_rec(ptr, end_ptr, page, page_zip, index);
			}
			break;
		...
		case MLOG_COMP_LIST_START_DELETE:
			...
			if (NULL != (ptr = mlog_parse_index(ptr, end_ptr, type == MLOG_COMP_LIST_END_DELETE || type == MLOG_COMP_LIST_START_DELETE, &index))) {
				ut_a(!page || (ibool) !!page_is_comp(page) == dict_table_is_comp(index->table));
...
)	 // !< in: end lsn of the mtr
{
	...
	/* Store the log record body in chunks of less than IB_PAGE_SIZE:
...
)	 // !< in/out: buffer block
{
	...
	while (recv) {
		...
		if (recv->type == MLOG_INIT_FILE_PAGE) {
			...
			memset(IB_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + page, 0, 8);
		...
		if (recv->start_lsn >= page_lsn) {
			...
			mach_write_ull(IB_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + page, end_lsn);
...
void recv_apply_hashed_log_recs(
	// =======================
	ibool allow_ibuf
) /*!< in: if TRUE, also ibuf operations are
...
// ================================
{
	...
	for (i = 0; i < n_hash_cells; i++) {
		...
		while (recv_addr != NULL) {
			...
			buf_page_init_for_backup_restore(recv_addr->space, recv_addr->page_no, zip_size, block);
			...
			if (zip_size) {
				error = fil_io(OS_FILE_READ, TRUE, recv_addr->space, zip_size, recv_addr->page_no, 0, zip_size, block->page.zip.data, NULL);
				if (error == DB_SUCCESS && !buf_zip_decompress(block, TRUE)) {
					srv_panic(DB_ERROR, "decompressing page");
				}
			} else {
				error = fil_io(OS_FILE_READ, TRUE, recv_addr->space, 0, recv_addr->page_no, 0, IB_PAGE_SIZE, block->frame, NULL);
			}
			...
			buf_flush_init_for_writing(block->frame, buf_block_get_page_zip(block), mach_read_ull(block->frame + FIL_PAGE_LSN));

			if (zip_size) {
				error = fil_io(OS_FILE_WRITE, TRUE, recv_addr->space, zip_size, recv_addr->page_no, 0, zip_size, block->page.zip.data, NULL);
			} else {
				error = fil_io(OS_FILE_WRITE, TRUE, recv_addr->space, 0, recv_addr->page_no, 0, IB_PAGE_SIZE, block->frame, NULL);
			}
		skip_this_recv_addr:
			recv_addr = HASH_GET_NEXT(addr_hash, recv_addr);
		}
				...
				added, log block headers not included */
{
	...
	lsn_len = (ulint)len;
	lsn_len += (lsn_len + frag_len) / (OS_FILE_LOG_BLOCK_SIZE - LOG_BLOCK_HDR_SIZE - LOG_BLOCK_TRL_SIZE) * (LOG_BLOCK_HDR_SIZE + LOG_BLOCK_TRL_SIZE);
					...
					this lsn */
{
	...
	do {
		...
		*/
		...
		if ((store_to_hash || (data_len == OS_FILE_LOG_BLOCK_SIZE)) && scanned_lsn + data_len > recv_sys->scanned_lsn && (recv_sys->scanned_checkpoint_no > 0) && (log_block_get_checkpoint_no(log_block) < recv_sys->scanned_checkpoint_no) && (recv_sys->scanned_checkpoint_no - log_block_get_checkpoint_no(log_block) > 0x80000000UL)) {
					...
					this lsn */
{
	...
	while (!finished) {
		...
		finished = recv_scan_log_recs(recovery, (buf_pool->curr_size - recv_n_pool_free_frames) * IB_PAGE_SIZE, TRUE, log_sys->buf, RECV_SCAN_SIZE, start_lsn, contiguous_lsn, group_scanned_lsn);
		start_lsn = end_lsn;
	}
...
) /*!< in/out: log group that contains the
					maximum consistent checkpoint */
{
	...
	fil_io(OS_FILE_READ | OS_FILE_LOG, TRUE, max_cp_group->space_id, 0, 0, 0, LOG_FILE_HDR_SIZE, log_hdr_buf, max_cp_group);
	...
	if (0 == ut_memcmp(log_hdr_buf + LOG_FILE_WAS_CREATED_BY_HOT_BACKUP, (byte *)"ibbackup", (sizeof "ibbackup") - 1)) {
		...
		fil_io(OS_FILE_WRITE | OS_FILE_LOG, TRUE, max_cp_group->space_id, 0, 0, 0, OS_FILE_LOG_BLOCK_SIZE, log_hdr_buf, max_cp_group);
	}
}
...
#define LIMIT_LSN IB_UINT64_T_MAX
	...
	if (TYPE_CHECKPOINT) {
		...
		if (recv_sys->scanned_lsn > checkpoint_lsn + capacity || checkpoint_lsn > recv_sys->scanned_lsn + capacity) {
					...
					divisible by OS_FILE_LOG_BLOCK_SIZE */
{
	...
	*/
	...
	for (i = 0; i < n_log_files; i++) {
		...
		ut_snprintf(name, sizeof(name), "%s%s%lu", log_dir, ib_logfile_basename, (ulong)i);
	...
	ut_snprintf(name, sizeof(name) - 1, "%s%s%lu", log_dir, ib_logfile_basename, (ulong)0);
...
try_open_again:
	...
	fil_node_create(name, 1 + file_size / IB_PAGE_SIZE, group->archive_space_id, FALSE);
...
#error "RECV_SCAN_SIZE < LOG_FILE_HDR_SIZE"
	...
	for (;;) {
		...
		ret = recv_scan_log_recs(
			...
			(buf_pool->curr_size - recv_n_pool_free_frames) * IB_PAGE_SIZE,
			TRUE,
			buf,
			len,
			start_lsn,
