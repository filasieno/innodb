...
IB_INTERN trx_purge_t *purge_sys = NULL;

/** A dummy undo record used as a return value when we have a whole undo log which needs no purge */
IB_INTERN trx_undo_rec_t trx_purge_dummy_rec;

/*****************************************************************/ /**
Reset the variables. */
IB_INTERN void trx_purge_var_init(void)
{
	purge_sys = NULL;
...
in: undo number
*/
static trx_undo_inf_t *trx_purge_arr_store_info(trx_id_t trx_no, undo_no_t undo_no)
{
	trx_undo_inf_t *cell;
	trx_undo_arr_t *arr;
	ulint i;
...
in: purge array
out: transaction number: ut_dulint_zero
if array is empty
out: undo number
*/
static void trx_purge_arr_get_biggest(trx_undo_arr_t *arr, trx_id_t *trx_no, undo_no_t *undo_no)
{
	trx_undo_inf_t *cell;
...
/*======================*/
{
	...
	purge_sys->view = read_view_oldest_copy_or_open_new(ut_dulint_zero, purge_sys->heap);
}
...
in: rollback segment
in: the file address of log_hdr
in: count of how many undo logs we will cut off from the end of the history list
*/
...
loop:
	...
	undo_page = trx_undo_page_get(rseg->space, rseg->zip_size, hdr_addr.page, &mtr);
...
) /*!< in: rollback segment */
{
	...
	undo_page = trx_undo_page_get_s_latched(rseg->space, rseg->zip_size, rseg->last_page_no, &mtr);
	...
	log_hdr = trx_undo_page_get_s_latched(rseg->space, rseg->zip_size, prev_log_addr.page, &mtr) + prev_log_addr.boffset;
...
/*===========================*/
{
	...
	if (!min_rseg->last_del_marks) {
		...
		rec = trx_undo_get_first_rec(space, zip_size, page_no, offset, RW_S_LATCH, &mtr);
		if (rec == NULL) {
			/* Undo log empty */
...
) /*!< in: memory heap where copied */
{
	...
	*cell = trx_purge_arr_store_info(purge_sys->purge_trx_no, purge_sys->purge_undo_no);
	...
	/* The following call will advance the stored values of purge_trx_no and purge_undo_no, therefore we had to store them first */
...
in: storage cell 
*/
IB_INTERN void trx_purge_rec_release(trx_undo_inf_t *cell)
{
	mutex_enter(&(purge_sys->mutex));
	trx_undo_arr_t *arr = purge_sys->arr;
...
*/
IB_INTERN ulint trx_purge(void)
{
	que_thr_t *thr;
	...
	// Determine how much data manipulation language (DML) statements need to be delayed in order to reduce the lagging of the purge thread.
	srv_dml_needed_delay = 0; /* in microseconds; default: no delay */
	// If we cannot advance the 'purge view' because of an old 'consistent read view', then the DML statements cannot be delayed. Also, srv_max_purge_lag <= 0 means 'infinity'
	if (srv_max_purge_lag > 0 && !UT_LIST_GET_LAST(trx_sys->view_list)) {
		...
		{
			// If the history list length exceeds the innodb_max_purge_lag, the data manipulation statements are delayed by at least 5000 microseconds.
	...
	ut_ad(thr);
	/*	thr2 = que_fork_start_command(purge_sys->query); ut_ad(thr2); */
	mutex_exit(&kernel_mutex);
