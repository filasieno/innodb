...
/// \brief This struct is placed first in every allocated memory block
...
/// \brief The value of ut_mem_block_struct::magic_n.  Used in detecting
/// memory corruption.
#define UT_MEM_MAGIC_N 1601650166

/// \brief List of all memory blocks allocated from the operating system
/// with malloc.  Protected by ut_list_mutex.
static UT_LIST_BASE_NODE_T(ut_mem_block_t) ut_mem_block_list;

/// \brief Flag: has ut_mem_block_list been initialized?
static ibool ut_mem_block_list_inited = FALSE;

/// \brief A dummy pointer for generating a null pointer exception in
/// ut_malloc_low()
...
/// \details NUL-terminating the result. Returns strlen(src), so truncation occurred if the return value >= size.
...
/// \brief Replace every occurrence of s1 in str with s2.
