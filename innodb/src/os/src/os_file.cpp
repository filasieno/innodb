...
/// @file os_file.cpp
/// \brief The interface to the operating system file i/o primitives
///
...
IB_INTERN ulint	os_innodb_umask
...
IB_INTERN os_mutex_t	os_file_seek_mutexes[OS_FILE_N_SEEK_MUTEXES];
...
IB_INTERN ibool	os_aio_use_native_aio	= FALSE;

/** Flag: enable debug printout for asynchronous i/o */
IB_INTERN ibool	os_aio_print_debug	= FALSE;
...
IB_INTERN ulint	os_n_file_writes	= 0;
IB_INTERN ulint	os_n_fsyncs		= 0;
IB_INTERN ulint	os_n_file_reads_old	= 0;
IB_INTERN ulint	os_n_file_writes_old	= 0;
IB_INTERN ulint	os_n_fsyncs_old		= 0;
IB_INTERN time_t	os_last_printout;
...
IB_INTERN ulint	os_file_n_pending_pwrites = 0;
/** Number of pending write operations */
IB_INTERN ulint	os_n_pending_writes = 0;
/** Number of pending read operations */
IB_INTERN ulint	os_n_pending_reads = 0;
...
static
os_aio_slot_t*
os_aio_array_get_nth_slot(os_aio_array_t* array, ulint index)
{
...
/// \brief Shutdown the IO sub-system and free all the memory.
IB_INTERN
					...
					and this parameter is TRUE */
{
	...
	if (err == OS_FILE_DISK_FULL) {
		...
		if (name) {
			ib_log(state, "InnoDB: File name %s\n", name);
		}

		ib_log(state, "InnoDB: File operation call: '%s'.\n",
			operation);
...
#if 0
	ib_log(state, "Opening file %s, mode %s, type %s, purpose %s\n",
...
# if 0
		ib_log(state, "Using O_SYNC for file %s\n", name);
# endif
	...
	*success = TRUE;
	...
	if (create_mode != OS_FILE_OPEN_RAW && os_file_lock(file, name)) {
		...
		if (create_mode == OS_FILE_OPEN_RETRY) {
			int i;
			ut_print_timestamp(state->stream);
			ib_log(state, "  InnoDB: Retrying to lock"
			      " the first data file\n");
			for (i = 0; i < 100; i++) {
	...
	ulint		size_high)/*!< in: most significant 32 bits of size */
{
	...
	if (desired_size >= (ib_int64_t)(100 * 1024 * 1024)) {
		...
		ib_log(state, "InnoDB: Progress in MB:");
	}
				...
				array */
{
	...
	/* ib_log(state, "Array n per seg %lu\n", n_per_seg); */
	...
	ib_stream_t	state->stream)	/*!< in: stream where to print */
{
	...
	for (i = 0; i < srv_n_file_io_threads; i++) {
		ib_log(state, "I/O thread %lu state: %s (%s)", (ulong) i,
	...
	ib_log(state, "Pending normal aio reads:");
...
loop:
	...
	for (i = 0; i < array->n_slots; i++) {
		...
		if (slot->reserved) {
			n_reserved++;
#if 0
			ib_log(state, "Reserved slot, messages %p %p\n",
				(void*) slot->message1,
				(void*) slot->message2);
#endif
			ut_a(slot->len > 0);
	...
	ib_log(state, " %lu", (ulong) n_reserved);

	os_mutex_exit(array->mutex);

	if (array == os_aio_read_array) {
		ib_log(state, ", aio writes:");
