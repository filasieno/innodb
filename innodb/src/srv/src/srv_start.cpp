...
IB_INTERN ibool srv_parse_data_file_paths_and_sizes(const char* usr_str)
{
	...
	input_str = str;
	// First calculate the number of data files and check syntax: path:size[M | G];path:size[M | G]... . Note that a Windows path may contain a drive name and a ':'
	while (*str != '\0') {
		path = str;
	...
	if (i == 0) {
		// If data_file_path was defined it must contain at least one data file definition 
		return(FALSE);
	}
	...
	srv_data_file_is_raw_partition = malloc(i * sizeof *srv_data_file_is_raw_partition);
	srv_n_data_files = i;
	...
	while (*str != '\0') {
		...
		if (*str == 'r' && *(str + 1) == 'a' && *(str + 2) == 'w') { str += 3;
...
char* input_str;
	...
	i = 0;
	input_str = str;
	// First calculate the number of directories and check syntax: path;path;... 
	while (*str != '\0') {
		path = str;
		while (*str != ';' && *str != '\0') {
			str++;
	...
	if (i != 1) {
		// If log_group_home_dir was defined it must contain exactly one path definition.
		return FALSE;
	}
...
IB_INTERN void srv_free_paths_and_sizes()
{
	...
	static os_thread_ret_t io_handler_thread(
		void* arg) /*!< in: pointer to the number of the segment in the aio array */
	{
		ulint segment;
		ulint i;
		segment = *((ulint*)arg);
		...
		ib_log(state, "Io handler thread %lu starts, id %lu\n", segment, os_thread_pf(os_thread_get_curr_id()));
	#endif
		for (i = 0;; i++) {
	...
	IB_INTERN void srv_normalize_path_for_win(
...
@return high 32 bytes of file size when expressed in bytes */
	/*!< in: file size in database pages */
...
static ulint open_or_create_log_file(
	ibool create_new_db, /*!< in: TRUE if we should create a new database */
	ibool* log_file_created, /*!< out: TRUE if new log file created */
	ibool log_file_has_been_opened,/*!< in: TRUE if a log file has been opened before: then it is an error to try to create another log file */
	...
	ulint i) /*!< in: log file number in group */
{
	...
	*log_file_created = FALSE;
	ut_a(ut_strlen(srv_log_group_home_dirs[k]) < (sizeof name) - 10 - sizeof "ib_logfile");
	...
	if (ret == FALSE) {
		if (os_file_get_last_error(FALSE) != OS_FILE_ALREADY_EXISTS
			...
			ib_log(state, "InnoDB: Error in creating or opening %s\n", name);
			return DB_ERROR;
		}

		files[i] = os_file_create(name, OS_FILE_OPEN, OS_FILE_AIO, OS_LOG_FILE, &ret);
		if (!ret) {
			ib_log(state, "InnoDB: Error in opening %s\n", name);
			return DB_ERROR;
		}
		...
		*log_file_created = TRUE;
		...
		ib_log(state, "InnoDB: Setting log file %s size to %lu MB\n", name, (ulong) srv_log_file_size >> (20 - IB_PAGE_SIZE_SHIFT));
		ib_log(state,
			"InnoDB: Database physically writes the file full: wait...\n");

		ret = os_file_set_size(name, files[i], srv_calc_low32(srv_log_file_size), srv_calc_high32(srv_log_file_size));
		if (!ret) {
			ib_log(state, "InnoDB: Error in creating %s: probably out of disk space\n", name);
			return(DB_ERROR);
		}
	}
	...
	/* If this is the first log group, create the file space object for archived logs.  */
	...
	if (i == 0) {
		log_group_init(k, srv_n_log_files, srv_log_file_size * IB_PAGE_SIZE, 2 * k + SRV_LOG_SPACE_FIRST_ID, SRV_LOG_SPACE_FIRST_ID + 1); /* dummy arch space id */
	}
	return DB_SUCCESS ;
}
...
@return DB_SUCCESS or error code */
static ulint open_or_create_data_files(ibool* create_new_db, /*!< out: TRUE if new database should be created */
#ifdef IB_LOG_ARCHIVE
					...
					new files added */
{
	...
	if (srv_n_data_files >= 1000) {
		ib_log(state, "InnoDB: can only have < 1000 data files; you have defined %lu\n", (ulong) srv_n_data_files);
		return DB_ERROR;
	}
	*sum_of_new_sizes = 0;
	*create_new_db = FALSE;
	...
	// We require that the user have a trailing '/' when setting the srv_data_home variable. */
	for (i = 0; i < srv_n_data_files; i++) {
		ibool is_absolute = FALSE;
		const char* ptr = srv_data_file_names[i];
#ifdef __WIN__
		// We are not using isalpha() here because of locale dependent issues.
		if (((*ptr >= 'a' && *ptr <= 'z') || (*ptr >= 'A' && *ptr <= 'Z') && *(ptr + 1) == ':') || *ptr == '\\' || *ptr == '/') {
			is_absolute = TRUE;
		}
#else
		...
		// If the name is not absolute then the system files are created relative to home.
		if (!is_absolute) {
			ut_a(ut_strlen(home) + ut_strlen(srv_data_file_names[i]) < (sizeof name) - 1);
		...
		if (srv_data_file_is_raw_partition[i] == 0) {
			// First we try to create the file: if it already exists, ret will get value FALSE 
			...
			// The partition is opened, not created; then it is written over
		...
		if (ret == FALSE) {
			...
			if (one_created) {
				ib_log(state, "InnoDB: Error: data files can only be added at the end\n");
				ib_log(state, "InnoDB: of a tablespace, but data file %s existed beforehand.\n", name);
				return DB_ERROR;
			}
			...
			rounded_size_pages = (size / (1024 * 1024) + 4096 * size_high) << (20 - IB_PAGE_SIZE_SHIFT);
...
skip_size_check:
			...
			if (i > 0) {
				ut_print_timestamp(state->stream);
				ib_log(state, "  InnoDB: Data file %s did not exist: new to be created\n", name);
			} else {
				ib_log(state, "InnoDB: The first specified data file %s did not exist:\nInnoDB: a new database to be created!\n", name);
				*create_new_db = TRUE;
			}

			ut_print_timestamp(state->stream);
			ib_log(state, "  InnoDB: Setting file %s size to %lu MB\n", name, (ulong) (srv_data_file_sizes[i] >> (20 - IB_PAGE_SIZE_SHIFT)));
			ib_log(state, "InnoDB: Database physically writes the" " file full: wait...\n");
...
static void srv_startup_abort(enum db_err err)
{
...
@return DB_SUCCESS or error code */
IB_INTERN ib_err_t innobase_start_or_create(void)
{
	buf_pool_t* ret;
	ibool create_new_db;
...
# endif /* IB_ZIP_COPY */
	...
	if (srv_buf_pool_size >= 1000 * 1024 * 1024) {
		...
		srv_max_n_threads = 1000; /* saves several MB of memory, especially in 64-bit computers */
	}
#endif
	err = srv_boot();
	if (err != DB_SUCCESS) {
	...
	/* file_io_threads used to be user settable, now it's just a sum of read_io_threads and write_io_threads */
	...
	if (srv_buf_pool_size <= 5 * 1024 * 1024) {
		ib_log(state, "InnoDB: Warning: Small buffer pool size "
	...
	if (0 != ut_strcmp(srv_log_group_home_dirs[0], srv_arch_dir)) {
		ib_log(state, "InnoDB: Error: you must set the log group home dir same as log arch dir.\n");
		return DB_ERROR;
	}
#endif // IB_LOG_ARCHIVE
	if (srv_n_log_files * srv_log_file_size >= 262144) {
		ib_log(state, "InnoDB: Error: combined size of log files  must be < 4 GB\n");
		return DB_ERROR ;
	}
	sum_of_new_sizes = 0;
	for (i = 0; i < srv_n_data_files; i++) {
		...
		if (sizeof(off_t) < 5 && srv_data_file_sizes[i] >= 262144) {
			ib_log(state,
				"InnoDB: Error: file size must be < 4 GB with this binary\n"
	...
	if (sum_of_new_sizes < 10485760 / IB_PAGE_SIZE) {
		ib_log(state "InnoDB: Error: tablespace size must be  at least 10 MB\n");
		return DB_ERROR;
	}
	...
	if (ret == NULL) {
		...
		fil_close();
		os_aio_close();
		ib_log(state, "InnoDB: Fatal error: cannot allocate the memory for the buffer pool\n");
		return DB_ERROR ;
	}
	...
	// Open all log files and data files in the system tablespace: we keep them open until database shutdown 
	...
	if (create_new_db) {
		...
		ib_log(state, "InnoDB: Starting archive recovery from a backup...\n");
		...
		// Initialize the fsp free limit global variable in the log system
		fsp_header_get_free_limit();
		// recv_recovery_from_checkpoint_finish needs trx lists which are initialized in trx_sys_init_at_db_start(). 
	...
	if (!create_new_db && sum_of_new_sizes > 0) {
		...
		mtr_commit(&mtr);
		/* Immediately write the log record about increased tablespace size to disk, so that it is durable even if we crash quickly */
	...
	// ib_log(state, "Max allowed record size %lu\n", page_get_free_space_of_empty() / 2); 
	// Create the thread which watches the timeouts for lock waits 
	os_thread_create(&srv_lock_timeout_thread, NULL, thread_ids + 2 + SRV_MAX_N_IO_THREADS);
	
	// Create the thread which warns of long semaphore waits 
	os_thread_create(&srv_error_monitor_thread, NULL, thread_ids + 3 + SRV_MAX_N_IO_THREADS);

	// Create the thread which prints InnoDB monitor info 
	os_thread_create(&srv_monitor_thread, NULL, thread_ids + 4 + SRV_MAX_N_IO_THREADS);
	...
	if (!srv_auto_extend_last_data_file
		...
		if (srv_force_recovery == IB_RECOVERY_DEFAULT && sum_of_data_file_sizes < tablespace_size_in_header) {
			...
			ib_log(state,
				"InnoDB: Cannot start InnoDB. The tail of the system tablespace is missing. Have you set the data_file_path in an inappropriate way, removing ibdata files from there?\n"
	...
	if (srv_print_verbose_log) {
		ut_print_timestamp(state->stream);
		ib_log(state," HailDB %s started; log sequence number %llu\n", VERSION, srv_start_lsn);
	}
	if (srv_force_recovery != IB_RECOVERY_DEFAULT) {
		ib_log(state, "InnoDB: !!! force_recovery is set to %lu !!!\n", (ulong) srv_force_recovery);
	}
...
static ibool srv_threads_try_shutdown(os_event_t lock_timeout_thread_event)
{
	...
	/* srv error monitor thread exits automatically, no need to do anything here */
	/* We wake the master thread so that it exits */
	srv_wake_master_thread();
	/* Exit the i/o threads */
	os_aio_wake_all_threads_at_shutdown();
	os_mutex_enter(os_sync_mutex);
	if (state->os.thread_count == 0) {
...
static ibool srv_threads_shutdown(void)
{
	...
	ib_log(state, "InnoDB: Warning: %lu threads created by InnoDB  had not exited at shutdown!\n", (ulong) state->os.thread_count);
	return(FALSE);
}

/** Shuts down the InnoDB database. 
@return DB_SUCCESS or error code */
IB_INTERN db_err innobase_shutdown(ib_shutdown_t shutdown) /*!< in: shutdown flag */
{
#ifdef __NETWARE__
	...
	/* This is currently required to inform the master thread only. Once we have contexts we can get rid of this global. */
	...
	/* This variable should come from the user and should not be malloced by InnoDB. */
	srv_data_home = NULL;
#ifdef IB_LOG_ARCHIVE
	free(srv_arch_dir);
	srv_arch_dir = NULL;
	...
	pars_lexer_var_init();
	ut_free_all_mem();
	if (state->os.thread_count != 0 || state->os.event_count != 0 || state->os.mutex_count != 0 || state->os.fast_mutex_count != 0) {
		ib_log(state, 
			"InnoDB: Warning: some resources were not  cleaned up in shutdown:\n"
			"InnoDB: threads %lu, events %lu, os_mutexes %lu, os_fast_mutexes %lu\n",
			(ulong) state->os.thread_count, (ulong) state->os.event_count,
			(ulong) state->os.mutex_count, (ulong) state->os.fast_mutex_count);
	}
	...
	if (srv_print_verbose_log) {
		ut_print_timestamp(state->stream);
		ib_log(state, "  InnoDB: Shutdown completed; log sequence number %llu\n", srv_shutdown_lsn);
	}
