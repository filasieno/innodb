...
IB_INTERN const charset_t* ib_ucode_get_connection_charset(void);
...
IB_INTERN int ib_utf8_strcasecmp(const char* a, const char* b);
...
/// \return 0 if a=b, <0 if a<b, >1 if a>b.
IB_INTERN int ib_utf8_strncasecmp(const char* a, const char* b, ulint n);
...
IB_INTERN void ib_utf8_convert_from_table_id(const charset_t* cs, char* to, const char* from, ulint to_len);
...
IB_INTERN void ib_utf8_convert_from_id(const charset_t* cs, char* to, const char* from, ulint to_len);
