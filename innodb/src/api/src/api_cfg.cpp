...
#include "api_api.hpp"
#include "btr_sea.hpp"
#include "buf_buf.hpp" // for buf_pool 
#include "buf_lru.hpp" // for buf_LRU_* 
#include "db_err.hpp"
#include "log_recv.hpp"
#include "srv_srv.hpp"
#include "srv_start.hpp"
#include "trx_sys.hpp"	// for trx_sys_file_format_name_to_id() 
#include "os_sync.hpp"
...
typedef enum ib_cfg_flag {
	...
	IB_CFG_FLAG_READONLY_AFTER_STARTUP = 0x2,/* can be modified only before innodb is started */
...
typedef struct ib_cfg_var {
	...
	ib_uint64_t	min_val;/* minimum allowed value for numeric types, ignored for other types */
	ib_uint64_t	max_val;/* maximum allowed value for numeric types, ignored for other types */
	ib_err_t (*validate)(const struct ib_cfg_var*, const void*); /* function used to validate a new variable's value when setting it */
	ib_err_t (*set)(struct ib_cfg_var*, const void*); /* function used to set the variable's value */
	ib_err_t (*get)(const struct ib_cfg_var*, void*); /* function used to get the variable's value */
	void* tank;	/* opaque storage that may be used by the set() and get() functions */
} ib_cfg_var_t;


/// \brief Assign src to dst according to type. If this is a string variable (char*)
/// the string itself is not copied.
/// \detail ib_cfg_assign()
/// \param type type of src and dst
...
static ib_err_t ib_cfg_assign(ib_cfg_type_t type, void* dst, const void* src)
{
	...
	// do not add default: in order to produce a compilation warning if new type is added which is not handled here
	}
	// UNREACHABLE
	return DB_ERROR;
}

/// \brief A generic function used for ib_cfg_var_t::validate() to check a numeric
/// type for min/max allowed value overflow.
/// \detail ib_cfg_var_validate_numeric()
/// \param cfg_var configuration variable to check
...
/// \brief A generic function used for ib_cfg_var_t::set() that stores the value
...
/// \brief A generic function used for ib_cfg_var_t::get() that retrieves the value
...
/// \brief Set the value of the config variable "adaptive_hash_index".
/// \detail ib_cfg_var_set_adaptive_hash_index()
/// \param cfg_var in/out: configuration variable to manipulate, must be "adaptive_hash_index"
/// \param value in: value to set, must point to ib_bool_t variable
/// \return DB_SUCCESS if set successfully
static ib_err_t ib_cfg_var_set_adaptive_hash_index(struct ib_cfg_var* cfg_var, const void* value)
{
...
/// \brief Retrieve the value of the config variable "adaptive_hash_index".
/// \detail ib_cfg_var_get_adaptive_hash_index()
/// \param cfg_var in: configuration variable whose value to retrieve, must be "adaptive_hash_index"
/// \param value out: place to store the retrieved value, must point to ib_bool_t variable
/// \return DB_SUCCESS if retrieved successfully
static ib_err_t ib_cfg_var_get_adaptive_hash_index(const struct ib_cfg_var* cfg_var, void* value)
{
	ut_a(strcasecmp(cfg_var->name, "adaptive_hash_index") == 0);
	ut_a(cfg_var->type == IB_CFG_IBOOL);
...
/// \brief Set the value of the config variable "data_file_path".
/// \detail ib_cfg_var_set_data_file_path()
...
/// \brief Retrieve the value of the config variable "data_file_path".
/// \detail ib_cfg_var_get_data_file_path()
...
/// \brief Set the value of the config variable "file_format".
/// \detail ib_cfg_var_set_file_format()
...
/// \brief Retrieve the value of the config variable "file_format".
/// \detail ib_cfg_var_get_file_format
...
/// \brief Check the value of the config variable "data_home_dir". We need to ensure
/// that the value ends with a path separator.
/// \detail ib_cfg_var_validate_data_home_dir()
...
/// \brief Set the value of the config variable "log_group_home_dir".
/// \detail ib_cfg_var_set_log_group_home_dir
...
/// \brief Set the value of the config variable "flush_method".
/// \detail ib_cfg_var_set_flush_method
/// \param cfg_var in/out: configuration variable to manipulate, must be "flush_method"
/// \param value in: value to set, must point to char* variable
/// \return DB_SUCCESS if set successfully
...
/// \brief Retrieve the value of the config variable "log_group_home_dir".
/// \detail ib_cfg_var_get_log_group_home_dir()
...
/// \brief Set the value of the config variable "lru_old_blocks_pct".
/// \detail ib_cfg_var_set_lru_old_blocks_pct()
...
/// \brief Retrieve the value of the config variable "version".
/// \detail ib_cfg_var_get_version()
...
static const ib_cfg_var_t cfg_vars_defaults[] = {
	...
	},
	{
		...
		STRUCT_FLD(max_val,   0),
		STRUCT_FLD(validate,  NULL),
		STRUCT_FLD(set,       NULL ), /* The ::set() function should never be called because this variable is flagged as IB_CFG_FLAG_READONLY */
		STRUCT_FLD(get,       ib_cfg_var_get_generic),
	...
	},
	{
		...
		STRUCT_FLD(max_val,   0),
		STRUCT_FLD(validate,  NULL),
		STRUCT_FLD(set,       NULL), // The ::set() function should never le is flagged as IB_CFG_FLAG_READONLY 
...
/// \brief This mutex has to work even if the InnoDB latching infrastructure hasn't been initialized
static os_fast_mutex_t cfg_vars_mutex;
static ib_cfg_var_t	cfg_vars[UT_ARR_SIZE(cfg_vars_defaults)];


/// \brief Lookup a variable name.
/// \detail ib_cfg_lookup_var()
/// \param var in: variable name
/// \return config variable instance if found else NULL
static ib_cfg_var_t* ib_cfg_lookup_var(const char* var)
{
	ulint i;
...
/// \brief Get the type of a configuration variable. Returns DB_SUCCESS if the
/// variable with name "name" was found and "type" was set.
/// \detail ib_cfg_var_get_type()
/// \param name in: variable name
/// \param type out: variable type
...
/// \brief Set a configuration variable. "ap" must contain one argument whose type
...
/// was set.
/// \detail ib_cfg_set_ap()
/// \param name in: variable name
/// \param ap in: variable value
...
static ib_err_t ib_cfg_set_ap(const char* name, va_list ap)
{
	...
	cfg_var = ib_cfg_lookup_var(name);
	if (cfg_var != NULL) {
		// check whether setting the variable is appropriate, according to its flag
		if (cfg_var->flag & IB_CFG_FLAG_READONLY || (cfg_var->flag & IB_CFG_FLAG_READONLY_AFTER_STARTUP && srv_was_started)) {
			...
			switch (cfg_var->type) {
			case IB_CFG_IBOOL: {
				ib_bool_t value = va_arg(ap, ib_bool_t);
				ret = cfg_var->set(cfg_var, &value);
				break;
			}
			case IB_CFG_ULINT: {
				ulint value = va_arg(ap, ulint);
				ret = cfg_var->set(cfg_var, &value);
				break;
			}
			case IB_CFG_ULONG: {
				ulong value = va_arg(ap, ulong);
				ret = cfg_var->set(cfg_var, &value);
				break;
			}
			case IB_CFG_TEXT: {
			...
			case IB_CFG_CB: {
				ib_cb_t	value = va_arg(ap, ib_cb_t);
				ret = cfg_var->set(cfg_var, &value);
				break;
			}
			// do not add default: in order to produce a compilation warning if new type is added which is not handled here
...
/// \brief Set a configuration variable. The second argument's type depends on the
...
/// \brief Get the value of a configuration variable. The type of the returned value
...
/// \detail ib_cfg_get()
/// \param name in: variable name
/// \param value out: pointer to the place to store the retrieved value
/// \return DB_SUCCESS if retrieved successfully
...
/// \brief Get a list of the names of all configuration variables.
...
/// \detail ib_cfg_get_all()
/// \param names out: pointer to array of strings
/// \param names_num out: number of strings returned
