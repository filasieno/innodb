...
/// \def UNIV_NO_IGNORE
/// \brief Some HailDB methods will produce warnings if the result of them is not
...
/// \brief @struct ib_col_meta_t InnoDB column meta data.
typedef struct {
	/// \brief Column type
	ib_col_type_t	type;
...
/// \brief ib_trx_state_t The transaction state can be queried using the
...
/// \brief The first argument to the InnoDB message logging function. By default
...
/// \ingroup ddl
/// \param ib_tbl_sch is the table schema to delete
...
/// \param ib_trx is the current transaction, can be NULL
...
/// \brief Type of callback in the event of HailDB panicing. Your callback should
...
/// You may specify a callback that HailDB will check during certain wait
...
/// \ingroup trx
/// \param ib_trx Transaction where error occurred
...
/// \brief Inject an error into HailDB
/// \ingroup debug
