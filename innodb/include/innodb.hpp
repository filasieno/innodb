...
/// \def UNIV_NO_IGNORE
/// \brief Some INNODB methods will produce warnings if the result of them is not
...
/// \brief The boolean value of "true" used internally within InnoDB
#define IB_TRUE	 (0x1UL)
/// \brief The boolean value of "false" used internally within InnoDB
#define IB_FALSE (0x0UL)

// Basic types used by the InnoDB API.

/// \brief All InnoDB error codes are represented by ib_err_t. 
...
typedef enum {
	...
	/// \brief The configuration parameter is a callback parameter.
	IB_CFG_CB
} ib_cfg_type_t;
...
/// \brief Add a BLOB column to a table schema.
...
/// \brief Add an UNSIGNED INT column to a table schema.
/// \param s is the schema handle
/// \param n is the column name
/// \return DB_SUCCESS or error code
#define ib_tbl_sch_add_u32_col(s, n) ib_table_schema_add_col(s, n, IB_INT, IB_COL_UNSIGNED, 0, 4)

/// \brief Add an UNSIGNED BIGINT column to a table schema.
/// \param s is the schema handle
/// \param n is the column name
/// \return DB_SUCCESS or error code
#define ib_tbl_sch_add_u64_col(s, n) ib_table_schema_add_col(s, n, IB_INT, IB_COL_UNSIGNED, 0, 8)

/// \brief Add an UNSIGNED BIGINT NOT NULL column to a table schema.
/// \param s is the schema handle
/// \param n is the column name
/// \return DB_SUCCESS or error code
#define ib_tbl_sch_add_u64_notnull_col(s, n) ib_table_schema_add_col(s, n, IB_INT, IB_COL_NOT_NULL | IB_COL_UNSIGNED,0, 8)

/// \brief Set an int configuration variable.
/// \ingroup config
...
/// \brief Set a text configuration variable.
/// \ingroup config
/// \param name is the config variable name
...
/// \brief Set a boolean configuration variable to IB_TRUE.
/// \ingroup config
/// \param name is the config variable name
/// \return DB_SUCCESS or error code
...
/// \brief Set a boolean configuration variable to IB_FALSE.
/// \ingroup config
/// \param name is the config variable name
/// \return DB_SUCCESS or error code
#define ib_cfg_set_bool_off(name) ib_cfg_set(name, IB_FALSE)

/// \brief Set a generic ib_cb_t callback function.
/// \ingroup config
/// \param name is the config variable name
...
INNODB_API ib_u64_t ib_api_version(void) UNIV_NO_IGNORE;
...
/// \ingroup init
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_init(void) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error code
/// \see DB_SUCCESS
INNODB_API ib_err_t ib_startup(const char* format) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or error code
INNODB_API ib_err_t ib_shutdown(ib_shutdown_t flag) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_trx_start(ib_trx_t trx, ib_trx_level_t ib_trx_level) UNIV_NO_IGNORE;
...
/// \param ib_trx_level is the transaction isolation level
/// \return	innobase txn handle
INNODB_API ib_trx_t ib_trx_begin(ib_trx_level_t ib_trx_level) UNIV_NO_IGNORE;
...
INNODB_API ib_trx_state_t ib_trx_state(ib_trx_t	trx) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_trx_release(ib_trx_t	trx) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_trx_commit(ib_trx_t trx) UNIV_NO_IGNORE;
...
/// \param trx is the transaction handle
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_trx_rollback(ib_trx_t trx) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_table_schema_add_col(ib_tbl_sch_t tbl_sch, const char* name, ib_col_type_t col_type, ib_col_attr_t col_attr, ib_u16_t client_type, ib_ulint_t len) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_table_schema_add_index(ib_tbl_sch_t tbl_sch, const char* name, ib_idx_sch_t* idx_sch) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_table_schema_create(const char* name, ib_tbl_sch_t* tbl_sch, ib_tbl_fmt_t tbl_fmt, ib_ulint_t page_size) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_index_schema_add_col(ib_idx_sch_t idx_sch, const char* name, ib_ulint_t prefix_len) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_index_schema_create(ib_trx_t usr_trx, const char* name, const char* table_name, ib_idx_sch_t* idx_sch) UNIV_NO_IGNORE;

/// \brief Set index as clustered index. Implies UNIQUE.
/// \ingroup ddl
/// \param[in,out] idx_sch is the index schema to update
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_index_schema_set_clustered(ib_idx_sch_t idx_sch) UNIV_NO_IGNORE;
...
/// \param[in,out] idx_sch is the index schema to update
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_index_schema_set_unique(ib_idx_sch_t idx_sch) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_table_create(ib_trx_t trx, const ib_tbl_sch_t sch, ib_id_t* id) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_table_rename(ib_trx_t trx, const char* old_name, const char* new_name) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_index_create(ib_idx_sch_t idx_sch, ib_id_t* index_id) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or err code
INNODB_API ib_err_t ib_table_drop(ib_trx_t trx, const char*	name) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_index_drop(ib_trx_t trx, ib_id_t index_id) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_open_table_using_id(ib_id_t table_id, ib_trx_t trx, ib_crsr_t* crsr) UNIV_NO_IGNORE;
...
/// \param[out] crsr is the new cursor
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_open_index_using_id(ib_id_t index_id, ib_trx_t trx, ib_crsr_t* crsr) UNIV_NO_IGNORE;
...
/// \param[out] crsr is the new cursor
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_open_index_using_name(ib_crsr_t open_crsr, const char* index_name, ib_crsr_t* crsr) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_open_table(const char* name, ib_trx_t trx, ib_crsr_t* crsr) UNIV_NO_IGNORE;

/// \brief Reset the cursor.
/// \ingroup cursor
/// \param crsr is an open cursor
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_reset(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Close an InnoDB table and free the cursor.
/// \ingroup cursor
/// \param crsr is an open cursor
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_close(ib_crsr_t crsr) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_insert_row(ib_crsr_t crsr, const ib_tpl_t tpl) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_update_row(ib_crsr_t crsr, const ib_tpl_t ib_old_tpl, const ib_tpl_t ib_new_tpl) UNIV_NO_IGNORE;

/// \brief Delete a row in a table.
/// \ingroup dml
/// \param crsr is the cursor instance
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_delete_row(ib_crsr_t crsr) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_read_row(ib_crsr_t crsr, ib_tpl_t tpl) UNIV_NO_IGNORE;

/// \brief Move cursor to the prev user record in the table.
/// \ingroup cursor
/// \param crsr is the cursor instance
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_prev(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Move cursor to the next user record in the table.
/// \ingroup cursor
/// \param crsr is the cursor instance
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_next(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Move cursor to the first record in the table.
/// \ingroup cursor
/// \param crsr is the cursor instance
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_first(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Move cursor to the last record in the table.
/// \ingroup cursor
/// \param crsr is the cursor instance
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_last(ib_crsr_t crsr) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or err code
INNODB_API ib_err_t ib_cursor_moveto(ib_crsr_t crsr, ib_tpl_t tpl, ib_srch_mode_t ib_srch_mode, int* result) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_col_set_value(ib_tpl_t tpl, ib_ulint_t col_no, const void* src, ib_ulint_t len) UNIV_NO_IGNORE;
...
INNODB_API ib_ulint_t ib_col_get_len(ib_tpl_t tpl, ib_ulint_t i) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_tuple_read_i8(ib_tpl_t tpl, ib_ulint_t i, ib_i8_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_u8(ib_tpl_t tpl, ib_ulint_t i, ib_u8_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_i16(ib_tpl_t tpl, ib_ulint_t i, ib_i16_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_u16(ib_tpl_t tpl, ib_ulint_t i, ib_u16_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return	DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_i32(ib_tpl_t tpl, ib_ulint_t i, ib_i32_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return	DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_u32(ib_tpl_t tpl, ib_ulint_t i, ib_u32_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return	DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_i64(ib_tpl_t tpl, ib_ulint_t i, ib_i64_t* ival) UNIV_NO_IGNORE;
...
/// \param[out] ival is the integer value
/// \return	DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_u64(ib_tpl_t tpl, ib_ulint_t i, ib_u64_t* ival) UNIV_NO_IGNORE;
...
INNODB_API const void* ib_col_get_value(ib_tpl_t tpl, ib_ulint_t i) UNIV_NO_IGNORE;
...
INNODB_API ib_tpl_t ib_tuple_clear(ib_tpl_t	tpl) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or error code
INNODB_API ib_err_t ib_tuple_get_cluster_key(ib_crsr_t crsr, ib_tpl_t* dst, const ib_tpl_t src) UNIV_NO_IGNORE;
...
/// \return	DB_SUCCESS or error code
INNODB_API ib_err_t ib_tuple_copy(ib_tpl_t dst, const ib_tpl_t src) UNIV_NO_IGNORE;

/// \brief Create an InnoDB tuple used for index/table search.
/// \ingroup tuple
/// \param crsr is the cursor instance
/// \return tuple for current index
INNODB_API ib_tpl_t ib_sec_search_tuple_create(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Create an InnoDB tuple used for index/table search.
/// \ingroup tuple
/// \param crsr is the cursor instance
/// \return tuple for current index
INNODB_API ib_tpl_t ib_sec_read_tuple_create(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Create an InnoDB tuple used for table key operations.
/// \ingroup tuple
/// \param crsr is the cursor instance
/// \return tuple for current table
INNODB_API ib_tpl_t ib_clust_search_tuple_create(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Create an InnoDB tuple for table row operations.
/// \ingroup tuple
/// \param crsr is the cursor instance
/// \return Tuple for current table
INNODB_API ib_tpl_t ib_clust_read_tuple_create(ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Return the number of user columns in the tuple definition.
/// \ingroup tuple
/// \param tpl is a tuple
/// \return number of user columns
INNODB_API ib_ulint_t ib_tuple_get_n_user_cols(const ib_tpl_t tpl) UNIV_NO_IGNORE;

/// \brief Return the number of columns in the tuple definition.
/// \ingroup tuple
/// \param tpl is a tuple
/// \return number of columns
INNODB_API ib_ulint_t ib_tuple_get_n_cols(const ib_tpl_t tpl) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_cursor_truncate(ib_crsr_t* crsr, ib_id_t* table_id) UNIV_NO_IGNORE;
...
/// \param[out] table_id is the new table id
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_table_truncate(const char* table_name, ib_id_t* table_id) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_table_get_id(const char* table_name, ib_id_t* table_id) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS if found
INNODB_API ib_err_t ib_index_get_id(const char* table_name, const char* index_name, ib_id_t* index_id) UNIV_NO_IGNORE;

/// \brief Create a database if it doesn't exist.
/// \ingroup ddl
/// \param db_name is the name of the database to create
/// \return IB_TRUE on success
INNODB_API ib_bool_t ib_database_create(const char* db_name) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_database_drop(const char* db_name) UNIV_NO_IGNORE;

/// \brief Check if cursor is positioned.
/// \ingroup cursor
/// \param crsr is the cursor instance to check
/// \return IB_TRUE if positioned
INNODB_API ib_bool_t ib_cursor_is_positioned(const ib_crsr_t crsr) UNIV_NO_IGNORE;

/// \brief Latches the data dictionary in shared mode.
/// \ingroup ddl
/// \param trx is the transaction instance
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_schema_lock_shared(ib_trx_t trx) UNIV_NO_IGNORE;

/// \brief Latches the data dictionary in exclusive mode.
/// \ingroup ddl
/// \param trx is the transaction instance
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_schema_lock_exclusive(ib_trx_t trx) UNIV_NO_IGNORE;
...
INNODB_API ib_bool_t ib_schema_lock_is_exclusive(const ib_trx_t trx) UNIV_NO_IGNORE;

/// \brief Checks if the data dictionary is latched in shared mode.
/// \ingroup ddl
/// \param trx is a transaction instance
/// \return TRUE if shared latch
INNODB_API ib_bool_t ib_schema_lock_is_shared(const ib_trx_t trx) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_cursor_lock(ib_crsr_t crsr, ib_lck_mode_t mode) UNIV_NO_IGNORE;
...
/// \param mode is the lock mode
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_table_lock(ib_trx_t trx, ib_id_t table_id, ib_lck_mode_t mode) UNIV_NO_IGNORE;
...
/// \param mode is the lock mode
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_cursor_set_lock_mode(ib_crsr_t crsr, ib_lck_mode_t mode) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_table_schema_visit(ib_trx_t trx, const char* name, const ib_schema_visitor_t* visitor, void* arg) UNIV_NO_IGNORE;

/// \brief List all the tables in the InnoDB's data dictionary. It will abort if visitor returns a non-zero value. It will call the function: visitor.tables(arg, const char* name, int name_len); The function will abort if visitor.tables() returns non-zero.
/// \ingroup ddl
...
INNODB_API ib_err_t ib_schema_tables_iterate(ib_trx_t trx, ib_schema_visitor_table_all_t visitor, void* arg) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS if successful
INNODB_API ib_err_t ib_cfg_var_get_type(const char* name, ib_cfg_type_t* type) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_cfg_set(const char* name, ...) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_cfg_get(const char* name, void* value) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_cfg_get_all(const char*** names, ib_u32_t* names_num) UNIV_NO_IGNORE;
...
INNODB_API void ib_savepoint_take(ib_trx_t trx, const void* name, ib_ulint_t name_len);
...
INNODB_API ib_err_t ib_savepoint_release(ib_trx_t trx, const void* name, ib_ulint_t name_len) UNIV_NO_IGNORE;
...
/// \param name_len is the length of name in bytes
/// \return if no savepoint of the name found then DB_NO_SAVEPOINT, otherwise DB_SUCCESS
INNODB_API ib_err_t ib_savepoint_rollback(ib_trx_t trx, const void* name, ib_ulint_t name_len) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_i8(ib_tpl_t tpl, int col_no, ib_i8_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_i16(ib_tpl_t tpl, int col_no, ib_i16_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_i32(ib_tpl_t tpl, int col_no, ib_i32_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_i64(ib_tpl_t tpl, int col_no, ib_i64_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_u8(ib_tpl_t tpl, int col_no, ib_u8_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_u16(ib_tpl_t tpl, int col_no, ib_u16_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_u32(ib_tpl_t tpl, int col_no, ib_u32_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_u64(ib_tpl_t tpl, int col_no, ib_u64_t val) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_double(ib_tpl_t tpl, int col_no, double val) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_double(ib_tpl_t tpl, ib_ulint_t col_no, double* dval) UNIV_NO_IGNORE;
...
/// \param val is the value to write
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_write_float(ib_tpl_t tpl, int col_no, float val) UNIV_NO_IGNORE;
...
/// \return DB_SUCCESS or error
INNODB_API ib_err_t ib_tuple_read_float(ib_tpl_t tpl, ib_ulint_t col_no, float* fval) UNIV_NO_IGNORE;
...
INNODB_API const char* ib_strerror(ib_err_t db_errno) UNIV_NO_IGNORE;
...
INNODB_API ib_err_t ib_status_get_i64(const char* name, ib_i64_t* dst) UNIV_NO_IGNORE;
...
/// \param[out] names_num number of strings returned
/// \return DB_SUCCESS or error code
INNODB_API ib_err_t ib_status_get_all(const char*** names, ib_u32_t* names_num) UNIV_NO_IGNORE;
